...
var EasyPZLoader = /** @class */ (function () {
    ...
    EasyPZLoader.prototype.checkElements = function () {
        var els = document.querySelectorAll('[easypz]');
        var prevEls = this.easyPzElements.map(function (obj) { return obj.element; });
        for (var i = 0; i < els.length; i++) {
            var el = els[i];
            var settingsString = el.getAttribute('easypz');
...
var EasyPZ = /** @class */ (function () {
    ...
    EasyPZ.prototype.reset = function(){
	console.log(this.totalTransform.scale);
	this.totalTransform = { scale: 1, translateX: 0, translateY: 0 };
        this.totalTransformSnapshot = { scale: 1, translateX: 0, translateY: 0 };
	this.lastAppliedTransform = { translateX: 0, translateY: 0, scale: 1 };
    ...
    EasyPZ.prototype.setupHostListeners = function () {
        var _this = this;
        this.el.addEventListener('mousedown', function (event) { return _this.onMouseDown(event); });
        this.el.addEventListener('touchstart', function (event) { return _this.onTouchStart(event); });
        this.el.addEventListener('mousemove', function (event) { return _this.onMouseMove(event); });
        this.el.addEventListener('touchmove', function (event) { return _this.onTouchMove(event); });
        this.el.addEventListener('mouseup', function (event) { return _this.onMouseUp(event); });
        this.el.addEventListener('mouseout', function (event) { return _this.onMouseOut(event); });
        this.el.addEventListener('touchend', function (event) { return _this.onTouchEnd(event); });
        this.el.addEventListener('contextmenu', function (event) { return _this.onContextMenu(); });
        this.el.addEventListener('wheel', function (event) { return _this.onWheel(event); });
    };
