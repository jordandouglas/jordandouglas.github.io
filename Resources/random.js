...
RAND_JS.exponential = function(rate){
	return new Random(Math.ceil(MER_JS.random() * 1e6)).exponential(rate);
}

RAND_JS.gamma = function(rate, shape){
	return new Random(Math.ceil(MER_JS.random() * 1e6)).gamma(rate, shape);
}

RAND_JS.normal = function(mean, sd){
	return new Random(Math.ceil(MER_JS.random() * 1e6)).normal(mean, sd);
}

RAND_JS.uniform = function(lower, upper){
	return new Random(Math.ceil(MER_JS.random() * 1e6)).uniform(lower, upper);
}
...
if (typeof RUNNING_FROM_COMMAND_LINE !== 'undefined' && RUNNING_FROM_COMMAND_LINE){
	...
	module.exports = {
		exponential: RAND_JS.exponential,
		gamma: RAND_JS.gamma,
		normal: RAND_JS.normal,
		uniform: RAND_JS.uniform
	}
